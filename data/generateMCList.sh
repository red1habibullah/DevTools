betterDAS.py dataset --primaryDatasets\
 "DY1*pythia8" "DY2*pythia8" "DY3*pythia8" "DY4*pythia8" "DYJetsToLL_Pt*pythia8" "DYJetsToLL_M-50_Tune*pythia8" "DYJetsToLL_M-10to50_Tune*pythia8"\
 "HPlusPlus*"\
 "QCD_Pt_*0_Tune*pythia8" "QCD_Pt-*_DoubleEMEnriched_MGG-*_Tune*Pythia8"\
 "GJet_Pt-*_DoubleEMEnriched_MGG-*_Tune*Pythia8"\
 "DiPhotonJets*"
 "ST_s-*" "ST_t-*f_incl*" "ST_tW_*f_incl*"\
 "TTJets_SingleLeptFromT_Tune*" "TTJets_SingleLeptFromTbar_Tune*" "TTJets_DiLept_Tune*" "TTJets_Tune*"\
 "TGJets*" "TTGG_0Jets*" "TTGJets"\
 "W*JetsToLNu_Tune*" "WJetsToLNu_Tune*" "WJetsToLNu_Pt-*"\
 "WGG*" "WWG*" "WWW*" "WWZ*" "WZG*" "WZZ*" "ZZZ*" "ZGG*"\
 "WGToLNuG_Tune*"\
 "WWTo2L2Nu_13TeV-powheg" "WWToLNuQQ_13TeV-powheg" "WWTo4Q_13TeV-powheg"\
 "WZJ*" "WZToLNu2Q_13TeV_powheg_pythia8" "WZTo3LNu_Tune*" "WZTo*_13TeV_amcatnloFXFX_madspin_pythia8" "WZTo3LNu_*Jets*" "WLLJJ*"\
 "ZGTo2LG_Tune*"\
 "GluGluToContinToZZ*" "ZZJ*" "ZZTo*"\
 "tZq_ll_4f_13TeV-amcatnlo-pythia8"\
 "ttW*" "ttZ*"\
 "TTW*" "TTZ*"\
 --dataTiers "MINIAODSIM" --acquisitionEras "RunIISummer16*" --processNames "*PUMoriond17_80X_mcRun2_asymptotic_2016_TrancheIV_v6*" > datasetList_MC_new.txt
