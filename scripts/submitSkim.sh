...
while read sample; do
    ...
    if [[ $sample == *"Run2016H"* ]]; then
        ...
        submit_job.py crabSubmit --filesPerJob 4 --samples $sample --publish --applyLumiMask "Collisions16" "$NAME" DevTools/Ntuplizer/test/MuMuTauTau_cfg.py isMC=0 crab=1 numThreads=1 runH=1
    else
        ...
        submit_job.py crabSubmit --filesPerJob 3 --ignoreLocality --samples $sample --publish --applyLumiMask "Collisions16" "$NAME" DevTools/Ntuplizer/test/MuMuTauTau_cfg.py isMC=0 crab=1 numThreads=1 runH=0
    fi
