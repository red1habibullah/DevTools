...
### Combine objects ###
#######################
process.mumuPairs = cms.EDProducer("CandViewShallowCloneCombiner",
    decay = cms.string("{0}@+ {0}@-".format(collections['muons'])),
    cut   = cms.string(""),
)
objectCountFilter(process,process.main_path,'mumuPairs','mumuPairs',1)

process.tautauPairs = cms.EDProducer("CandViewShallowCloneCombiner",
    decay = cms.string("{0}@+ {0}@-".format(collections['taus'])),
    cut   = cms.string(""),
)

process.tautauPairsBoosted = cms.EDProducer("CandViewShallowCloneCombiner",
    decay = cms.string("{0}@+ {0}@-".format(collections['tausBoosted'])),
    cut   = cms.string(""),
)

process.mumutautau = cms.EDProducer("CandViewShallowCloneCombiner",
    decay = cms.string("mumuPairs tautauPairs"),
    cut   = cms.string(""),
)

process.mumutautauBoosted = cms.EDProducer("CandViewShallowCloneCombiner",
    decay = cms.string("mumuPairs tautauPairsBoosted"),
    cut   = cms.string(""),
)
...
process.out = cms.OutputModule('PoolOutputModule',
    ...
    outputCommands = cms.untracked.vstring(
        ...
        'keep *_mumuPairs_*_*',
        'keep *_tautauPairs_*_*',
        'keep *_tautauPairsBoosted_*_*',
        'keep *_mumutautau_*_*',
